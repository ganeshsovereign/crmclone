...
exports.install = function() {
    ...
    F.route('/erp/api/ordersfab/caFamily', object.caFamily, ['authorize']);
    F.route('/erp/api/ordersfab/statistic', object.statistic, ['post', 'json', 'authorize']);
    F.route('/erp/api/ordersfab/pdf/', object.pdfAll, ['post', 'json', 'authorize']);
    F.route('/erp/api/ordersfab/csv/', object.csvAll, ['post', 'json', 'authorize']);
    F.route('/erp/api/ordersfab/mvt/', object.csvMvt, ['post', 'json', 'authorize']);
    F.route('/erp/api/ordersfab/pdf/{ordersfabId}', object.pdf, ['authorize']);
    F.route('/erp/api/ordersfab/pdf/{ordersfabId}/{version}', function(ref, version) {
        object.pdf(ref + '/' + version, this);
    }, ['authorize']);

    // recupere la liste des courses pour verification
    F.route('/erp/api/ordersfab/billing', billing.read, ['authorize']);

    // Valid les BL en bloc
    F.route('/erp/api/ordersfab/validate', object.validAll, ['post', 'json', 'authorize']);
    // Genere la facturation des BL en groupe
    F.route('/erp/api/ordersfab/billing', billing.createAll, ['post', 'json', 'authorize']);

    F.route('/erp/api/ordersfab/billing/ca', billing.familyCA, ['authorize']);
    ...
    F.route('/erp/api/ordersfab/download/{:id}', object.download);
};
            ...
            async.each(docs, function(client, callback) {
                ...
                SocieteModel.findOne({ _id: client._id.societe }, function(err, societe) {
                    ...
                    var facture = new FactureModel({
                        title: {
                            ref: "FO" + moment(datec).format(CONFIG('dateformatShort')),
                            autoGenerated: true
                        },
                        client: {
