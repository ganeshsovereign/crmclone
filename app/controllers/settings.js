...
MetronicApp.controller('SettingEntityController', ['$rootScope', '$scope', '$http', '$timeout', 'superCache', 'Settings',
		function($rootScope, $scope, $http, $timeout, superCache, Settings) {
				...
				if (typeof superCache.get("SettingEntityController") !== "undefined") {
...
MetronicApp.controller('SettingIntegrationController', ['$rootScope', '$scope', '$http', '$timeout', function($rootScope, $scope, $http, $timeout) {
...
MetronicApp.controller('SettingModuleController', ['$rootScope', '$scope', '$http', '$timeout', 'superCache', 'Settings',
		function($rootScope, $scope, $http, $timeout, superCache, Settings) {
				$scope.docTemplates = {};
				$scope.$on('$viewContentLoaded', function() {
						Metronic.initAjax();
						...
						if ($rootScope.$stateParams.id && $rootScope.$state.current.name === "settings.module.show")
								return $scope.findOne();
				});
				...
				$scope.find = function() {
						...
						Settings.module.query(query, function(data) {
								var docs = data.data.values;
								var docTemplates = [];

								for (var doc in docs) {
										if (docs[doc].type === $rootScope.$stateParams.id)
												docTemplates.push({
														name: docs[doc].name,
														desc: docs[doc].desc,
														type: docs[doc].type,
														isActive: docs[doc].isActive,
														default: docs[doc].default
												});
								}
				...
				$scope.update = function(line) {
						var object = $scope.docTemplates;

						if (!object._id)
								return;

						console.log(object.values[line.name]);
						object.values[line.name] = line;
						/*object.$update(function(response) {
						    $scope.findOne();
						});*/
				};
				...
				$scope.findOne = function() {
						...
						Settings.module.get({
								...
								$scope.docTemplates = docTemplates;
								var docTemplates = $scope.docTemplates.values;
								var filter = [];

								for (var doc in docTemplates) {
										if (docTemplates[doc].type === $rootScope.$stateParams.id)
												filter.push({
														name: docTemplates[doc].name,
														desc: docTemplates[doc].desc,
														type: docTemplates[doc].type,
														isActive: docTemplates[doc].isActive,
														default: docTemplates[doc].default
												});
								}
								$scope.docTemplates.values = filter;
						});
				};

				$scope.setDefault = function(line) {
						console.log(line);
						$scope.docTemplates.forEach(function(doc) {
								doc.default = line === doc;
						});
